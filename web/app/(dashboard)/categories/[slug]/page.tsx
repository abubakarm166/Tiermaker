...
export default function CategoryDetailPage() {
  ...
  useEffect(() => {
    let cancelled = false;
    Promise.all([fetchCategory(slug), fetchTemplates({ category: slug })])
      .then(([cat, tmpl]) => {
        if (!cancelled) {
          setCategory(cat);
          setTemplates(tmpl);
  ...
  return (
    <div>
      <div className="flex gap-4 mb-6 items-center">
        ...
        <Link href={`/templates/new?category=${slug}`} className="btn-primary">
      ...
      {list.length === 0 ? (
        <div className="card p-8 text-muted text-center">
          ...
          <Link href={`/templates/new?category=${slug}`} className="link-primary hover:underline">
            Create one
          </Link>
        </div>
      ) : (
