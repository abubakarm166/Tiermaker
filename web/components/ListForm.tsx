...
export default function ListForm() {
  ...
  useEffect(() => {
    ...
    fetchList(slug)
      .then((list) => {
        ...
        const rows = template.tier_rows ?? [];
        setRowOrder(
          list.row_order && list.row_order.length
            ? list.row_order
  ...
  const moveRowUp = (displayIndex: number) => {
    ...
    setRowOrder((prev) => {
      const next = [...prev];
      [next[displayIndex - 1], next[displayIndex]] = [
        next[displayIndex],
        next[displayIndex - 1],
      ];
      return next;
  ...
  const moveRowDown = (displayIndex: number) => {
    ...
    setRowOrder((prev) => {
      const next = [...prev];
      [next[displayIndex], next[displayIndex + 1]] = [
        next[displayIndex + 1],
        next[displayIndex],
      ];
      return next;
  ...
  const unassigned = items.filter(
    (item) =>
      !displayRows.some((r) => (assignments[r.key] ?? []).includes(item.id))
  );
  ...
  const scheduleAutoScroll = (clientY: number) => {
    ...
    autoScrollRafRef.current = requestAnimationFrame(() => {
      ...
      const edge = 120; // px from viewport edge
      const maxSpeed = 22; // px per frame (fast enough without feeling jumpy)
  ...
  const handleDrop = (e: React.DragEvent, toLabel: string | null) => {
    ...
    if (!raw) return;
    try {
      const { itemId, fromLabel } = JSON.parse(raw) as {
        itemId: number;
        fromLabel: string | null;
      };
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {loading ? (
          <div className="text-muted py-8">Loading template…</div>
        ) : template ? (
          <>
            <p className="text-muted text-sm mb-2">
              Drag items into tiers or click tier labels. Use ↑/↓ to reorder
              tiers.
            </p>
                ...
                >
                    ...
                    {(assignments[row.key] ?? []).map((itemId) => {
                      ...
                      return (
                        <div
                          ...
                          draggable
                          onDragStart={(e) =>
                            handleDragStart(e, item.id, row.key)
                          }
                          onDragEnd={handleDragEnd}
                          className="w-14 h-14 rounded-lg overflow-hidden bg-surface-elevated border border-app hover:border-[#FF9F1C] shrink-0 cursor-grab active:cursor-grabbing focus:outline-none focus:ring-2 focus:ring-[#FF9F1C]"
                        ...
                        >
                          {item.image ? (
                            ...
                            <span className="flex items-center justify-center h-full text-muted text-xs pointer-events-none">
                              {item.name}
                            </span>
                          )}
                        </div>
                  ...
                  <div className="flex flex-col gap-0.5 shrink-0">
                    ...
                    <button
                      ...
                      className="p-1.5 rounded text-muted hover:text-white hover:bg-white/10 disabled:opacity-40 disabled:pointer-events-none"
                      title="Move tier up"
            ...
            {settingsRowDisplayIndex !== null &&
              displayRows[settingsRowDisplayIndex] && (
                <div
                  className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60"
                  onClick={() => setSettingsRowDisplayIndex(null)}
                >
                  <div
                    className="bg-surface-elevated border border-app rounded-xl shadow-xl max-w-md w-full p-6"
                  ...
                  >
                    ...
                    {(() => {
                      ...
                      return (
                        <>
                          <div className="flex items-center gap-3 mb-4 p-3 rounded-lg bg-surface">
                            <span
                              className="font-display font-semibold w-10 h-10 flex items-center justify-center rounded"
                              style={{
                          ...
                          <p className="text-muted text-sm mb-2">
                            Edit label text (shown on list):
                          </p>
                          <input
                            type="text"
                            value={labelOverrides[row.key] ?? row.label}
                            onChange={(e) =>
                              setRowDisplayLabel(row.key, e.target.value)
                            }
                          ...
                          <div className="flex flex-wrap gap-2">
                            <button
                              type="button"
                              onClick={() => {
                                moveRowUp(settingsRowDisplayIndex);
                                setSettingsRowDisplayIndex((prev) =>
                            ...
                            <button
                              type="button"
                              onClick={() => {
                                moveRowDown(settingsRowDisplayIndex);
                                setSettingsRowDisplayIndex((prev) =>
                                  prev != null && prev < displayRows.length - 1
                                    ? prev + 1
                                    : null
                                );
                              }}
                              disabled={
                                settingsRowDisplayIndex ===
                            ...
                            <button
                              ...
                              className="btn-secondary text-primary hover:bg-[#FF9F1C]/20"
                            >
            ...
            <div
              className={`mt-4 rounded-xl border-2 border-dashed p-4 transition-colors ${dragOverUnassigned ? "border-[#FF9F1C] bg-[#FF9F1C]/10" : "border-app bg-surface-elevated/50"}`}
                ...
                {unassigned.map((item) => (
                  <div key={item.id} className="flex flex-col items-center gap-2">
                    <div
                      ...
                      onDragEnd={handleDragEnd}
                      className="w-16 h-16 rounded-lg overflow-hidden bg-surface-elevated border border-app hover:border-[#333] shrink-0 cursor-grab active:cursor-grabbing"
                      role="button"
                      tabIndex={0}
                    >
                      {item.image ? (
                        ...
                        <span className="flex items-center justify-center h-full text-muted text-xs px-1 text-center pointer-events-none">
                          {item.name}
                        </span>
                      )}
                    </div>
                    <span className="text-muted-strong text-xs max-w-[80px] truncate text-center">
                      {item.name}
                    </span>
