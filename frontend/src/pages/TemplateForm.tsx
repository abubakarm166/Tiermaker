...
export default function TemplateForm() {
  ...
  const addRowAbove = (index: number) => {
    setRows((prev) => {
      const newRow: Omit<TierRow, 'id'> = { label: 'New', color: '#808080', order: index }
  ...
  const addRowBelow = (index: number) => {
    setRows((prev) => {
      const newRow: Omit<TierRow, 'id'> = { label: 'New', color: '#808080', order: index + 1 }
    ...
    <div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 max-w-2xl">
        ...
        <div>
          ...
          <p className="text-slate-500 text-xs mt-1">Cover image for the template card on the Templates and Categories pages.</p>
        </div>
        <div>
          ...
          <p className="text-slate-500 text-xs mt-1">Link this template to a category so it appears on the Categories page.</p>
        </div>
        <div>
        ...
        <div>
          <h3 className="text-slate-300 font-medium mb-2">Tier rows</h3>
          <p className="text-slate-500 text-sm mb-2">Reorder with ↑ / ↓. First row is top tier.</p>
          <ul className="space-y-2">
            {rows.map((r, i) => (
              <li
                key={i}
                className="flex items-center gap-2 p-2 rounded-lg border border-slate-700/50 bg-slate-800/50"
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60" onClick={() => setSettingsRowIndex(null)}>
            <div className="bg-slate-800 border border-slate-600 rounded-xl shadow-xl max-w-md w-full p-6" onClick={(e) => e.stopPropagation()}>
              <div className="flex justify-between items-start mb-4">
                ...
                <button type="button" onClick={() => setSettingsRowIndex(null)} className="p-1 rounded text-slate-400 hover:text-white hover:bg-slate-600" aria-label="Close">
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                </button>
              </div>
              <p className="text-slate-400 text-sm mb-2">Choose a Label Background Color:</p>
                ...
                {COLOR_SWATCHES.map((hex) => (
                  <button
                    ...
                    style={{
                      ...
                      boxShadow: rows[settingsRowIndex].color === hex ? '0 0 0 2px var(--tw-border-color)' : undefined,
                    }}
                    title={hex}
              ...
              <p className="text-slate-400 text-sm mb-2">Edit Label Text Below:</p>
              <input
                type="text"
              ...
              <div className="flex flex-wrap gap-2">
                <button type="button" onClick={() => deleteRow(settingsRowIndex)} className="btn-secondary text-rose-400 hover:bg-rose-500/20">
                  Delete Row
                </button>
                <button type="button" onClick={() => addRowAbove(settingsRowIndex)} className="btn-secondary">
                  Add a Row Above
                </button>
                <button type="button" onClick={() => addRowBelow(settingsRowIndex)} className="btn-secondary">
                  Add a Row Below
                </button>
              </div>
            </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-slate-300 font-medium">Items</h3>
            <button type="button" onClick={addItem} className="btn-secondary text-sm">Add item</button>
          </div>
          <ul className="space-y-3">
            {items.map((item, index) => (
              <li key={index} className="flex flex-wrap gap-2 items-start p-3 rounded-lg bg-slate-800/50">
                <input
                  value={item.name}
                ...
                <button type="button" onClick={() => removeItem(index)} className="btn-ghost text-rose-400">Remove</button>
              </li>
            ))}
          </ul>
        </div>
